...
| State management | Provider (9 ChangeNotifiers) | Lightweight, no boilerplate, Built for this scale [3] |
...
We structured state into nine single-responsibility providers, each extending `ChangeNotifier`:
...
2. **Pathway match (20)** — +20 if the user's academic pathway maps to the opportunity category (e.g., CS → hackathons, Business → entrepreneurship workshops)
...
The score is recalculated every time the feed loads, so changing your interests or skills immediately re-ranks the feed. There is no machine learning. It is just weighted matching in a Dart service class.
...
**Mock accounts instead of real auth.** We used a `_mockAccounts` map with seven preset users (Fatima the organiser, Kwame the club leader, Dr. Nkosi the staff member, etc.) keyed by email. This let us test different roles, campuses, and permission levels without building a backend. The trade-off is that anyone can log in as anyone by knowing the email. For a prototype it was the right call. For production this would move to Firebase Auth or a custom API.

**Client-side match scoring.** The algorithm runs entirely in Dart on the mock data. This keeps the prototype self-contained but means the match score is recalculated on every feed load rather than queried from a database. With 14 opportunities the cost is negligible, but it would need an indexed backend query at scale.

**Role not persisted on login.** Initially the login tab always assigned `role: 'Student'`, so organiser credentials never unlocked the dashboard or post functionality. We fixed this by adding the `_mockAccounts` map in `AuthProvider` that maps known email addresses to preset name/role/campus triples. Any email matching a key now receives the correct role; unknown emails default to Student.

**Bottom nav overflow.** A fixed-height `SizedBox` inside the `BottomAppBar` conflicted with system navigation bar insets, producing an 11-pixel RenderFlex overflow on devices with gesture navigation. We removed the fixed height constraint, set `BottomAppBar(padding: EdgeInsets.zero)`, and gave each `_NavItem` intrinsic height via `Padding(vertical: 8)`, allowing the system to allocate the correct safe-area space.

**Skills not in bottom nav.** The Skills Marketplace was initially reachable only via a named route, not from the bottom navigation, causing users to believe skill posting was restricted. We promoted Skills to a first-class bottom nav tab (index 2), placing it between Explore and the FAB notch where it is immediately visible.

**sqflite cold-start delay.** Seeding 14 opportunities, 10 skill listings, 54 feedback entries, and 60 chat messages in `onCreate` caused a noticeable delay on first launch. We batched all seed inserts into a single `db.batch().commit()` call, reducing individual write overhead and completing the seed in a single transaction.
...
AI tooling (Claude Code) was used in this project for code scaffolding, boilerplate generation, and debugging assistance. All architectural decisions, feature designs, and UX rationale in this report reflect the team's independent thinking and were developed through discussion and iteration. Team members are fully able to explain every implementation decision, modify the codebase live, and justify all product choices during evaluation. The match score algorithm, the role-split dashboard design, the dual-persistence strategy, and the Campus Pulse concept were all defined by the team before any code was generated.
...
Each team member maintains an individual feature branch. The project structure follows a layered architecture: `constants → models → services → providers → screens → widgets`, with files organised across 9 providers, 7 models, 3 services, 12 reusable widgets, and 19 screens.
